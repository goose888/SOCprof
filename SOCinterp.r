...
# Table fields: Profile ID, Veg Class, Lat, Lon, top, bottom, Layer depth, Soil Order, Suborder, Internal,
...
mishradata <- read.table("sel_sites_carbon.csv", header=TRUE, sep=",", quote="\"")
...
info <- mishradata[, c("Profile.ID","Veg_Class", "Lat_N","Long","top","bottom","nodedepth","Layer_depth", "Soil_Order",
...
depths(mishradata) <- Profile.ID ~ top + bottom   # Promote normal data.frame to SoilProfileCollection data frame
